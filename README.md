...
| **Image Tools** | Preview-first editing: every tool shows a live preview of the selected image. Resize by pixels or percent, compress for email (shows how much smaller the result is), crop by dragging with aspect presets, watermarks previewed as you adjust them, 2x-4x enlargement, rotate and flip. All batch, all local. |
| **Background removal** | An interactive editor, not just a button (macOS 14+): DeskPulse finds the subjects, then you click which ones to keep and fix the edges with Restore and Erase brushes over a live transparency preview. Batch auto mode too. |
